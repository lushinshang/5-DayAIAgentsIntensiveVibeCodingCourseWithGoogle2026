...
**指令 6（2026-06-20）**
```
使用imagegen skill，參考readme.md及其規範，生圖並更新index.html
```
...
- [x] **生圖執行**：使用內建 `imagegen` 完成 5 張 1672×941 PNG（圖 A–E），存放於 `images/`
- [x] **HTML 正式整合**：
  - 移除 5 處「佔位」註解與 `onerror` 靜默隱藏
  - 加入 `width="1672"`、`height="941"`、`loading="lazy"`、`decoding="async"`
  - 修正手機版將 16:9 圖片裁成 4:3 的 CSS，改為完整等比例顯示
- [x] **最終驗證**：
  - 5 張圖皆為有效 PNG，尺寸均為 1672×941
  - `python3 -m html.parser index.html` PASS
  - 桌機 1440×1000：5/5 載入，渲染 718×404，無水平溢位
  - 手機 390×844：5/5 載入，渲染 356×200，無水平溢位
...
所有圖片已於 2026-06-20 使用 `imagegen` skill 的內建 `image_gen` 工具逐張生成，並依上方檔名存入 `images/`。提示詞以本節規格為基準，執行時補充網頁資訊圖用途、文字可讀性、禁止浮水印與避免額外文字等約束。
