...
SRT_PATH = Path(
    "/Users/lanss/projects/2_Practice/readpaper/mvmake/subtitles/"
    "Whitepaper Companion Podcast Agent Skills.en-orig.srt"
)
OUT_DIR = ROOT / "podcast-deep-guide"
MD_PATH = OUT_DIR / "podcast-deep-guide.md"
HTML_PATH = OUT_DIR / "podcast-deep-guide.html"
SRT_COPY_PATH = OUT_DIR / "Whitepaper Companion Podcast Agent Skills.en-orig.srt"

VIDEO_URL = (
    "https://github.com/lushinshang/5-DayAIAgentsIntensiveVibeCodingCourseWithGoogle2026/"
    "releases/download/DAY_1_Livestream/"
    "Whitepaper.Companion.Podcast.Agent.Skills_compressed.mp4"
)
ANTIGRAVITY_URL = "https://codelabs.developers.google.com/getting-started-with-antigravity-skills"
AGENTS_CLI_URL = "https://codelabs.developers.google.com/agents-cli-adk-lifecycle"


ARTICLE_MD = """# Agent Skills Podcast 深度導讀：不要把整家公司塞進 Prompt
...
def parse_srt(srt: str) -> list[tuple[str, str, str]]:
    ...
    for block in blocks:
        ...
        if len(lines) < 3:
            continue
        if not lines[0].isdigit() or "-->" not in lines[1]:
            continue
        raw_text = " ".join(lines[2:])
        if raw_text.startswith(">>"):
            speaker = "專家" if speaker == "主持人" else "主持人"
        text = re.sub(r"^>>\s*", "", raw_text)
        text = re.sub(r"\s+", " ", text).strip()
        items.append((lines[1], speaker, text))
    return items
...
def markdown_to_html(md: str) -> str:
    ...
    for raw in md.splitlines():
        ...
        if line.startswith("# "):
            ...
            anchor = re.sub(r"[^\w]+", "-", line[3:].lower()).strip("-")
...
def build_html(article_html: str, transcript_items: list[tuple[str, str, str]]) -> str:
    transcript_html = "\n".join(
        '<div class="transcript-turn">'
        f'<span class="spk-badge {("host" if speaker == "主持人" else "expert")}">{html.escape(speaker)}</span>'
        f"<p><time>{html.escape(ts)}</time>{html.escape(text)}</p>"
        "</div>"
        for ts, speaker, text in transcript_items
    )
...
<head>
  ...
  <title>Agent Skills Podcast 深度導讀</title>
  <style>
    :root {{
      color-scheme: light;
      --ink:#202124;
    ...
    * {{ box-sizing:border-box; }}
    ...
    .back {{ color:var(--teal); text-decoration:none; font-weight:700; }}
    h1 {{ font-size:clamp(2rem,5vw,4.4rem); line-height:1.08; margin:24px 0 16px; letter-spacing:0; }}
    .dek {{ max-width:760px; font-size:1.08rem; color:#3f4749; }}
    nav {{
      position:sticky;
      top:0;
      z-index:5;
    ...
    nav .wrap {{ display:flex; gap:16px; overflow:auto; padding:10px 22px; }}
    nav a {{ white-space:nowrap; color:#29484c; text-decoration:none; font-size:.92rem; font-weight:700; }}
    ...
    .resource-box h2 {{ margin-top:0; font-size:1.08rem; }}
    .resource-box a {{
      display:block;
      color:var(--teal);
      font-weight:800;
    ...
    article h1 {{ font-size:clamp(1.9rem,4vw,3.1rem); margin-top:0; }}
    article h2 {{
      font-size:1.55rem;
      line-height:1.35;
      margin:42px 0 12px;
    ...
    summary {{ cursor:pointer; font-weight:800; color:#163f44; }}
    .transcript-turn {{
      display:grid;
    ...
    .spk-badge.host {{
      background:#176b7515;
      color:#176b75;
      border-color:#176b7540;
    }}
    .spk-badge.expert {{
      background:#ad7f2215;
      color:#856115;
      border-color:#ad7f2240;
    }}
    .transcript p {{
      margin:0;
      color:#333;
    ...
    @media(max-width:760px) {{
      ...
      .transcript-turn {{ grid-template-columns:1fr; gap:6px; }}
      .spk-badge {{ justify-self:start; }}
    }}
  </style>
</head>
<body>
  <header>
    <div class="wrap">
      <a class="back" href="../index.html">返回 Day 3 主頁</a>
      <h1>Agent Skills Podcast 深度導讀</h1>
      <p class="dek">從 context rot、progressive disclosure、trajectory scoring 到企業治理，這份導讀把 Podcast 的對話整理成可落地的 Agent Skills 工程判斷。</p>
    </div>
  </header>
  <nav>
    <div class="wrap">
      <a href="#skill-不是新模型-而是可版本控制的工作記憶">Skill 結構</a>
      <a href="#token-經濟學-context-太多會讓模型變笨">Token 經濟學</a>
      <a href="#失敗不是模型不聰明-而是-skill-沒被測好">評估</a>
      <a href="#從-demo-到-production-真正重的是基礎設施">Production</a>
      <a href="#明天可以怎麼開始">明天開始</a>
    </div>
  </nav>
  <main class="wrap">
    <section class="media-panel" aria-label="Podcast media and resources">
      <video controls preload="metadata" src="{html.escape(VIDEO_URL)}"></video>
      <aside class="resource-box">
        <h2>實作學習頻道</h2>
        <a href="{html.escape(ANTIGRAVITY_URL)}">Antigravity Skills Codelab</a>
        <a href="{html.escape(AGENTS_CLI_URL)}">Agents CLI + ADK Lifecycle Codelab</a>
        <a href="Whitepaper Companion Podcast Agent Skills.en-orig.srt">英文 SRT 逐字稿</a>
      </aside>
    </section>
    <article>
{article_html}
    ...
    <section class="transcript-section" id="transcript">
      <h2>英文原始逐字稿</h2>
      <div class="transcript-note">以下依 SRT 中的 <code>&gt;&gt;</code> 換 speaker 記號分為「主持人」與「專家」。原文內容僅移除 SRT 序號、時間軸外的 speaker 標記，未改寫英文句子。</div>
      <details>
...
{transcript_html}
    ...
    <footer>
      <p>本頁根據使用者提供的 Podcast SRT 與 Day 3 Agent Skills 白皮書主頁整理。導讀採繁體中文台灣用語，保留必要技術名稱。</p>
    </footer>
  </main>
</body>
</html>
...
def main() -> None:
    ...
    srt = SRT_PATH.read_text(encoding="utf-8-sig")
    transcript_items = parse_srt(srt)
    SRT_COPY_PATH.write_text(srt, encoding="utf-8")
    MD_PATH.write_text(
        ARTICLE_MD
        + "\n\n---\n\n"
        + "## 來源\n\n"
        + f"- Antigravity Skills Codelab：{ANTIGRAVITY_URL}\n"
        + f"- Agents CLI + ADK Lifecycle Codelab：{AGENTS_CLI_URL}\n"
        + "- 英文 SRT：`Whitepaper Companion Podcast Agent Skills.en-orig.srt`\n",
        encoding="utf-8",
    )
    article_html = markdown_to_html(ARTICLE_MD)
    HTML_PATH.write_text(build_html(article_html, transcript_items), encoding="utf-8")
    print(f"wrote {MD_PATH.relative_to(ROOT)}")
    print(f"wrote {HTML_PATH.relative_to(ROOT)}")
    print(f"copied {SRT_COPY_PATH.relative_to(ROOT)}")
    print(f"transcript cues {len(transcript_items)}")
